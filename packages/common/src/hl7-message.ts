...
export class HL7Message {
  ...
  parse(input: string | Buffer) {
    ...
    for (const [i, line] of lines.entries()) {
      ...
      try {
        ...
        this.header.field(MSHSegment.SendingApplication).value = headerItems[2];
        this.header.field(MSHSegment.SendingFacility).value = headerItems[3];
        this.header.field(MSHSegment.ReceivingApplication).value =
          headerItems[4];
        this.header.field(MSHSegment.ReceivingFacility).value = headerItems[5];
        this.header.field(MSHSegment.EncodingCharacters).value = encoding;
        this.header.field(MSHSegment.MessageControlID).value = headerItems[9];
        this.header.field(MSHSegment.VersionID).value = version;
        /* c8 ignore next */
  ...
  createAck(
    ...
    // Sending Application
    msh.field(MSHSegment.ReceivingApplication).value = msh.field(
      MSHSegment.SendingFacility,
    ).value;
    msh.field(MSHSegment.ReceivingFacility).value = this.header.field(
      MSHSegment.SendingFacility,
    ).value;
    msh.field(MSHSegment.MessageType).value = 'ACK';
    msh.field(MSHSegment.MessageType).component(2).value = this.header
      .field(MSHSegment.MessageType)
      .valueOf(2);
    msh.field(MSHSegment.MessageControlID).value = String(Date.now() + uid(5));

    const msa = out.addSegment('MSA');
    msa.field(MSASegment.AcknowledgmentCode).value = ackCode;
    msa.field(MSASegment.MessageControlID).value = this.header.field(
      MSHSegment.MessageControlID,
    ).value;
    if (textMessage) {
      if (textMessage instanceof HL7Error) {
        msa.field(MSASegment.TextMessage).value = textMessage.message;
        const err = out.addSegment('ERR');
        err.field(ERRSegment.HL7ErrorCode).value =
          textMessage.hl7ErrorCode || 207;
        err.field(ERRSegment.Severity).value = textMessage.severity;
        err.field(ERRSegment.ApplicationErrorCode).value =
          textMessage.appErrorCode;
        const errLocation = err.field(ERRSegment.ErrorLocation);
        errLocation.component(ERLType.SegmentID).value =
          textMessage.segmentType;
        errLocation.component(ERLType.SegmentSequence).value =
          textMessage.segmentSequence;
        errLocation.component(ERLType.FieldPosition).value =
          textMessage.fieldPosition;
        errLocation.component(ERLType.ComponentNumber).value =
          textMessage.componentPosition;
        errLocation.component(ERLType.FieldRepetition).value =
          textMessage.repetitionIndex;
        errLocation.component(ERLType.SubComponentNumber).value =
          textMessage.subComponentPosition;
      } else if (textMessage instanceof Error) {
        msa.field(MSASegment.TextMessage).value = textMessage.message;
        const err = out.addSegment('ERR');
        err.field(ERRSegment.HL7ErrorCode).value = 207;
        err.field(ERRSegment.Severity).value = 'E';
        if ((textMessage as any).code)
          err.field(ERRSegment.ApplicationErrorCode).value = (
            textMessage as any
          ).code;
      } else {
        msa.field(MSASegment.TextMessage).value = textMessage;
      }
    }
    return out;
  }
