...
export class Hl7Component {
  ...
  set value(value: any) {
    if (this._data) this._data.value = value;
    else this.subComponent(1)!.value = value;
  }
  ...
  subComponent(position: number = 1): Hl7SubComponent {
    if (this.field.isPrimitive)
  ...
  valueOf(subComponent?: number): any | undefined {
    if (this._data)
