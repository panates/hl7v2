...
import type { Hl7SubComponent } from './hl7-sub-component.js';
import {
  HL7FieldDefinition,
...
export class HL7Segment {
  ...
  subComponent(
    fieldPos: number,
    componentPos: number = 1,
    subComponentPos: number = 1,
    repetitionIndex: number = 0,
  ): Hl7SubComponent {
    return this.component(fieldPos, componentPos, repetitionIndex).subComponent(
      subComponentPos,
    );
  }

  defineField(position: number, def: HL7FieldDefinition) {
