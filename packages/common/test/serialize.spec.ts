...
describe('hl7v2:serialize', () => {
  it('should serialize number', function () {
    ...
    seg.field(ARQSegment.OccurrenceNumber).value = 1234;
  ...
  it('should serialize date', () => {
    ...
    seg.field(AL1Segment.IdentificationDate).value = new Date(
      '2001-01-12T15:30:00',
    );
  ...
  it('should serialize date/time', () => {
    ...
    seg.field(ABSSegment.DateTimeOfAttestation).value = new Date(
      '2001-01-12T15:30:00',
    );
    expect(seg.toHL7String()).toStrictEqual('ABS||||20010112153000');
  });

  it('should escape non chars', () => {
    const msg = new HL7Message(HL7Version.v2_5);
    const seg = msg.addSegment('ADD');
    seg.field(1).value = '|^~\\&abc\x09\x10d';
  ...
  it('should escape buffer', () => {
    const msg = new HL7Message(HL7Version.v2_5);
    const seg = msg.addSegment('ADD');
    seg.field(1).value = Buffer.from([9, 10, 32]);
  ...
  it('should escape TypedArray', () => {
    const msg = new HL7Message(HL7Version.v2_5);
    const seg = msg.addSegment('ADD');
    seg.field(1).value = new Uint8Array([9, 10, 32]);
    expect(seg.toHL7String()).toStrictEqual('ADD|\\X090a20\\');
  });
  ...
  it('should serialize repetitions', () => {
    ...
    seg.field(1).value = '1';
    seg.field(1).add().value = '2';
    seg.field(1).add().value = '';
    seg.field(1).add().value = null;
  ...
  it('should serialize components', () => {
    ...
    seg.field(MSHSegment.DateTimeOfMessage).value = undefined;
    seg.field(MSHSegment.MessageType).value = '1';
    seg.field(MSHSegment.MessageType).component(CM_MSGType.TriggerEvent).value =
      '2';
    expect(seg.toHL7String()).toStrictEqual(
      'MSH|^~\\&|||||||1^2|||2.5||||||UTF-8',
    );
